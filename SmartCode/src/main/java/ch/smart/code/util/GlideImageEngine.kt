...
class GlideImageEngine : ImageEngine {
            ...
            override fun setResource(resource: Bitmap?) {
                ...
                if (resource != null) {
                    ...
                    if (eqLongImage) {
                        // 加载长图
                        longImageView?.let {
                            it.isQuickScaleEnabled = true
                            it.isZoomEnabled = true
                            it.isPanEnabled = true
                            it.setDoubleTapZoomDuration(100)
                            it.setMinimumScaleType(SubsamplingScaleImageView.SCALE_TYPE_CENTER_CROP)
                            it.setDoubleTapZoomDpi(SubsamplingScaleImageView.ZOOM_FOCUS_CENTER)
                            it.setImage(
    ...
    override fun loadImage(context: Context, url: String, imageView: ImageView, longImageView: SubsamplingScaleImageView?) {
        this.loadImage(context, url, imageView, longImageView, null)
    }
    
    //加载相册目录
    override fun loadFolderImage(context: Context, url: String, imageView: ImageView) {
        Glide.with(context).asBitmap().load(url).override(180, 180).centerCrop().sizeMultiplier(0.5f)
    ...
    override fun loadGridImage(context: Context, url: String, imageView: ImageView) {
        Glide.with(context).load(url).override(200, 200).centerCrop()
                .apply(RequestOptions().placeholder(R.drawable.picture_image_placeholder))
                .into(imageView)
    }
    ...
    companion object {
        ...
        fun getInstance(): GlideImageEngine? {
            if (null == instance) {
                synchronized(GlideImageEngine::class.java) {
                    if (null == instance) {
