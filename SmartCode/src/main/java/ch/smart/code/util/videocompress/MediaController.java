package ch.smart.code.util.videocompress;
...
import ch.smart.code.util.videocompress.mp4.InputSurface;
import ch.smart.code.util.videocompress.mp4.MP4Builder;
import ch.smart.code.util.videocompress.mp4.Mp4Movie;
import ch.smart.code.util.videocompress.mp4.OutputSurface;
import ch.smart.code.util.videocompress.mp4.VideoInfo;
import ch.smart.code.util.videocompress.utils.VideoCompressorHandler;
...
public class MediaController {
    ...
    public CompressorListener compressorListener;
    ...
    public void scheduleVideoConvert(VideoInfo videoInfo, CompressorListener listener) {
        this.compressorListener = listener;
        isTermination = false;
    ...
    private void didWriteData(VideoInfo videoInfo, final File file, final boolean last, final boolean error) {
        ...
        VideoCompressorHandler.runOnUIThread(new Runnable() {
            @Override
            public void run() {
                if (error || last) {
                    synchronized (videoConvertSync) {
                ...
                if (error) {
                    if (compressorListener != null) {
                        compressorListener.error();
                    }
                } else {
                    if (firstWrite) {
                        if (compressorListener != null) {
                            compressorListener.start();
                        }
                    }
                    if (last) {
                        if (compressorListener != null) {
                            compressorListener.sucess(file.toString());
                        }
                    }
                }
            }
    ...
    private boolean startVideoConvertFromQueue() {
        if (!videoConvertQueue.isEmpty()) {
            ...
            disposable = VideoConvertRunnable.runConversion(videoInfo);
            return true;
        }
        return false;
    }
    ...
    private static class VideoConvertRunnable implements Runnable {
        ...
        public static Disposable runConversion(final VideoInfo videoInfo) {
            VideoConvertRunnable wrapper = new VideoConvertRunnable(videoInfo);
            return Schedulers.io().createWorker().schedule(wrapper);
