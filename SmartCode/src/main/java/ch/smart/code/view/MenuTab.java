 ...
 * @date 2016-01-27
 */
public class MenuTab extends HorizontalScrollView {
    // mode: 自适应宽度+滚动 / 均分
    ...
    public MenuTab(Context context, boolean hasIndicator) {
        this(context, null);
    ...
    public MenuTab addTab(Tab item) {
    ...
    /**
     * 根据 index 在对应的 Tab 上显示内容红点
     *
     * @param index   要显示内容红点的 Tab 的下标
     * @param content 要显示内容
     */
    public void showSignTextView(Context context, int index, String content) {
        if (content != null && content.trim().length() > 0) {
            Tab tab = getAdapter().getItem(index);
            tab.showSignTextView(context, content);
            notifyDataChanged();
        }
    }
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<MenuTab> mTabSegmentRef;

        public TabLayoutOnPageChangeListener(MenuTab tabSegment) {
            mTabSegmentRef = new WeakReference<>(tabSegment);
        }

        @Override
        public void onPageScrollStateChanged(final int state) {
            final MenuTab tabSegment = mTabSegmentRef.get();
            if (tabSegment != null) {
    ...
    public static class Tab {
        ...
        private int mSignCountMinSize = USE_TAB_SEGMENT;
        private int mSignCountMinSizeWithText = USE_TAB_SEGMENT;
        private int mSignCountPaddingHorizontal = USE_TAB_SEGMENT;
        ...
        public void setSignCountMinSize(int signCountMinSize) {
            this.mSignCountMinSize = signCountMinSize;
        }

        public void setSignCountMinSizeWithText(int signCountMinSizeWithText) {
            this.mSignCountMinSizeWithText = signCountMinSizeWithText;
        }

        public void setSignCountPaddingHorizontal(int signCountPaddingHorizontal) {
            this.mSignCountPaddingHorizontal = signCountPaddingHorizontal;
        }
        ...
        private TextView ensureSignCountView(Context context) {
            if (mSignCountTextView == null) {
                if (mSignCountMinSize == USE_TAB_SEGMENT) {
                    mSignCountMinSize = QMUIResHelper.getAttrDimen(context, R.attr.qmui_tab_sign_count_view_minSize);
                }
                if (mSignCountMinSizeWithText == USE_TAB_SEGMENT) {
                    mSignCountMinSizeWithText = QMUIResHelper.getAttrDimen(context, R.attr.qmui_tab_sign_count_view_minSize_with_text);
                }
                if (mSignCountPaddingHorizontal == USE_TAB_SEGMENT) {
                    mSignCountPaddingHorizontal = QMUIResHelper.getAttrDimen(context, R.attr.qmui_tab_sign_count_view_padding_horizontal);
                }
        ...
        public void showSignCountView(Context context, int count) {
            ...
            if (count != 0) {
                // 显示未读数
                signCountLp.height = mSignCountMinSizeWithText;
                signCountLp.width = ViewGroup.LayoutParams.WRAP_CONTENT;
                mSignCountTextView.setLayoutParams(signCountLp);
                mSignCountTextView.setMinHeight(mSignCountMinSizeWithText);
                mSignCountTextView.setMinWidth(mSignCountMinSizeWithText);
                mSignCountTextView.setText(getNumberDigitsFormattingValue(count));
            } else {
                // 显示红点
                signCountLp.height = mSignCountMinSize;
                signCountLp.width = mSignCountMinSize;
                mSignCountTextView.setLayoutParams(signCountLp);
                mSignCountTextView.setMinHeight(mSignCountMinSize);
                mSignCountTextView.setMinWidth(mSignCountMinSize);
         ...
         * 设置未读数或者小红点的文字大小
         *
         * @param textSize 单位：像素
         */
        public void setSignCountTextSize(int textSize) {
            mSignCountTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, textSize);
        }
         ...
         */
        public void showSignTextView(Context context, String content) {
            ensureSignCountView(context);
            mSignCountTextView.setVisibility(View.VISIBLE);
            RelativeLayout.LayoutParams signCountLp = (RelativeLayout.LayoutParams) mSignCountTextView.getLayoutParams();
            // 显示未读数
            ...
            mSignCountTextView.setMinHeight(mSignCountMinSizeWithText);
            mSignCountTextView.setMinWidth(mSignCountMinSizeWithText);
            mSignCountTextView.setPaddingRelative(mSignCountPaddingHorizontal, 0, mSignCountPaddingHorizontal, 0);
            mSignCountTextView.setText(content);
        }
